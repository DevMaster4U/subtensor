...
use crate::{
	...
	peer_connection_map::PeerConnectionMap,
...
pub struct Litep2pNetworkBackend {
	...
	/// Live connection multiaddrs (shared with [`Litep2pNetworkService`]).
	peer_connection_map: PeerConnectionMap,
}
...
impl<B: BlockT + 'static, H: ExHashT> NetworkBackend<B, H> for Litep2pNetworkBackend {
		...
		Self: Sized,
	{
		...
		let peer_connection_map = PeerConnectionMap::new();
			...
			tokio::select! {
				...
				event = self.litep2p.next_event() => match event {
					Some(Litep2pEvent::ConnectionEstablished { peer, endpoint }) => {
						...
						// log::info!(
						// 	target: LOG_TARGET,
						// 	"PEER_MAP peer_id={} addr={}",
						// 	peer_id,
						// 	addr,
						// );
						self.peer_connection_map.record(peer_id, addr);
					...
					Some(Litep2pEvent::ConnectionClosed { peer, connection_id }) => {
						...
						if context.num_connections == 0 {
							...
							self.peer_connection_map.remove(&peer_id);
